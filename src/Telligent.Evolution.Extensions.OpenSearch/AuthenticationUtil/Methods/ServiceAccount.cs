...
namespace Telligent.Evolution.Extensions.OpenSearch.AuthenticationUtil.Methods
{
    public class ServiceAccount : Authentication, IEncrypted
    {
        ...
        {
            const string ResourceFileName = "OpenSearch.Authentication.xml";
            CustomValidator customValidator = new CustomValidator
            {
                ControlToValidate = controlToValidate.ID,
                ErrorMessage = ResourceManager.GetString(errorMsg, ResourceFileName),
                Text = "*",
        ...
        private AuthenticationParam ParseValue(Control markup, AuthenticationParam parameter)
        {
            TextBox parameterValue = markup.FindControl(this.Name + parameter.Name) as TextBox;
            parameter.Value = (parameterValue != null) ? parameterValue.Text : String.Empty;
            return parameter;
        }

        private void AddParameter(Control panel, AuthenticationParam parameter, TextBox tb)
        {
            panel.Controls.Add(new HtmlGenericControl("div") { InnerText = parameter.Text });
            tb.ID = this.Name + parameter.Name;
            if (tb.TextMode == TextBoxMode.Password)
            {
                EncodePassword(tb, parameter.Value);
            }
            else
            {
